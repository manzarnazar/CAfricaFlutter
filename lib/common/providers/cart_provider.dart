...
class CartProvider extends ChangeNotifier {
  ...
  void addToCart(CartModel cartModel,
      [int isWholesale =
          0, // Optional positional parameter with a default value
      int wholesaleMinQty =
          0, // Optional positional parameter with a default value
      int wholesaleMaxQty = 0]) {
    // print(isWholesale);
    if (wholesaleMinQty > 0) {
      cartModel.quantity = wholesaleMinQty;
    }
  ...
  void priceOndetailScreen(CartModel cartModel,
      [int isWholesale =
          0, // Optional positional parameter with a default value
      int wholesaleMinQty =
          0, // Optional positional parameter with a default value
  ...
  void setqty(
      ...
      BuildContext? context,
      int isWholesale = 0,
      int wholesaleMinQty = 1,
      int wholesaleMaxQty = 0}) {
    if (isWholesale == 1) {
      _quantity = wholesaleMinQty;
    }
    // _cartList[index!].quantity = wholesaleMinQty;
    // _amount = _amount + _cartList[index].discountedPrice!;
    // if (showMessage) {
    //   showCustomSnackBarHelper('quantity_increase_from_cart'.tr,
    //       isError: false);
    ...
    // cartList[index!].quantity = _cartList[index].quantity! + 1;
    // _amount = _amount + _cartList[index].discountedPrice!;
    // cartRepo!.addToCartList(_cartList);
