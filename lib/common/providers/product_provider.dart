...
class ProductProvider extends ChangeNotifier {
  ...
  ProductModel? _wholesaleProductModel; // Added for wholesale products
  ProductModel? _collaborationProductModel; // Added for wholesale products
  ...
  ProductModel? get wholesaleProductModel =>
      _wholesaleProductModel; // Getter for wholesale products
  ProductModel? get collaborationProductModel =>
      _collaborationProductModel; // Getter for wholesale products
  ...
  Future<void> getItemList(int offset,
    ...
    if (offset == 1) {
      ...
      _wholesaleProductModel = null; // Reset wholesale products
      _collaborationProductModel = null; // Reset wholesale products
      ...
      if (offset == 1) {
        if (productType == ProductType.dailyItem) {
          ...
          _mostViewedProductModel =
              ProductModel.fromJson(apiResponse.response?.data);
        } else if (productType == ProductType.wholesaleItem) {
          // Handling wholesale products
          _wholesaleProductModel =
              ProductModel.fromJson(apiResponse.response?.data);
        } else if (productType == ProductType.collaborationItem) {
          // Handling wholesale products
          _collaborationProductModel =
              ProductModel.fromJson(apiResponse.response?.data);
        }
      } else {
        if (productType == ProductType.dailyItem) {
          ...
          // Handling wholesale products
          _wholesaleProductModel?.offset =
              ProductModel.fromJson(apiResponse.response?.data).offset;
          _wholesaleProductModel?.totalSize =
              ProductModel.fromJson(apiResponse.response?.data).totalSize;
          _wholesaleProductModel?.products?.addAll(
              ProductModel.fromJson(apiResponse.response?.data).products ?? []);
        } else if (productType == ProductType.collaborationItem) {
          // Handling wholesale products
          _collaborationProductModel?.offset =
              ProductModel.fromJson(apiResponse.response?.data).offset;
          _collaborationProductModel?.totalSize =
              ProductModel.fromJson(apiResponse.response?.data).totalSize;
          _collaborationProductModel?.products?.addAll(
              ProductModel.fromJson(apiResponse.response?.data).products ?? []);
        }
      }
    } else {
